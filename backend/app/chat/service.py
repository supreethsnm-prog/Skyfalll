...
_SYSTEM_PROMPT = (
    ...
    "get_historical_weather looks up a past weather reading (temperature, "
    "precipitation, wind, pressure) for a specific city and date from ERA5 "
    "reanalysis data. It only covers a small, fixed, pre-seeded set of major "
    "Indian cities and sample dates — not arbitrary locations or dates — and "
    "each reading is a single 12:00 UTC (17:30 IST) snapshot, not a daily "
    "mean/max or a 24-hour precipitation total. precip_mm specifically is the "
    "1-hour accumulation ending at the observation time (12:00 UTC / 17:30 IST), "
    "not a daily total — a very low or zero value does not mean the whole day "
    "was dry. If it reports no data for the "
    "requested city/date, tell the user that specific combination isn't in the "
    "pre-loaded historical dataset — never imply historical data doesn't exist "
